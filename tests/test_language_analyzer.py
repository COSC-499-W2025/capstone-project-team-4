...
class TestConstants:
    ...
    def test_constants_values(self):
        ...
        assert Constants.DEFAULT_MAX_SIZE == 1_000_000
        assert Constants.DEFAULT_MIN_SIZE == 1
        expected_dirs = {'.git', '__pycache__', 'node_modules'}
        assert expected_dirs.issubset(Constants.SKIP_DIRECTORIES)
        assert '.gitignore' in Constants.DEFAULT_HIDDEN_EXCEPTIONS
...
class TestFileAnalyzer:
    ...
    def test_language_detection(self, file_analyzer):
        ...
        test_config = {
            'filename_patterns': {'dockerfile': 'Docker'},
            'extensions': {'.py': 'Python', '.js': 'JavaScript'}
        }
        
        with patch.object(file_analyzer.config, '_config', test_config):
            ...
            with patch.object(FileUtils, 'get_file_info', return_value=('dockerfile', '')):
                assert file_analyzer.detect_language_by_extension('/path/Dockerfile') == 'Docker'
