...
export default function PortfolioPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8f7f4", color: "#1e293b", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif" }}>
      <style>{`
    ...
    `}</style>
      ...
      {isLoading && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "60vh", gap: 16 }}>
          <Loader2 style={{ width: 36, height: 36, color: "#0d9488" }} className="animate-spin" />
          <p style={{ color: "#94a3b8", fontSize: 14 }}>Generating your portfolio...</p>
        </div>
      )}
      ...
      {!isLoading && error && (
        <div style={{ maxWidth: 600, margin: "60px auto", padding: "0 24px" }}>
          <div style={{ background: "#fff1f2", border: "1px solid #fecdd3", borderRadius: 12, padding: "18px 20px", display: "flex", gap: 12, alignItems: "center" }}>
            <AlertCircle style={{ color: "#e11d48", width: 18, height: 18, flexShrink: 0 }} />
      ...
      {!isLoading && !error && portfolio && (
        <>
          ...
          <div style={{
            ...
            background: "linear-gradient(160deg, #f0fdfa 0%, #e6faf7 40%, #f8f7f4 100%)",
            overflow: "hidden",
            borderBottom: "1px solid #e2e8f0",
          }}>
            {/* Subtle teal texture blobs */}
            <div style={{ position: "absolute", top: -60, left: -40, width: 380, height: 380, background: "radial-gradient(circle, rgba(13,148,136,0.07) 0%, transparent 70%)", pointerEvents: "none" }} />
            <div style={{ position: "absolute", bottom: 0, right: "8%", width: 280, height: 280, background: "radial-gradient(circle, rgba(99,102,241,0.05) 0%, transparent 70%)", pointerEvents: "none" }} />
            {/* Decorative dot grid */}
            <div style={{
            ...
            <div style={{ maxWidth: 960, margin: "0 auto", position: "relative", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 48, flexWrap: "wrap" }}>
              ...
              <div style={{ flex: 1, minWidth: 260, textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center" }}>
                ...
                <h1 style={{ fontSize: "clamp(28px, 5vw, 46px)", fontWeight: 700, lineHeight: 1.15, margin: "0 0 16px", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", color: "#0f172a" }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button className={`pf-mode-btn ${mode === "public" ? "active" : ""}`} onClick={() => setMode("public")}>
                    🌐 Public

                  </button>
                  <button className={`pf-mode-btn ${mode === "private" ? "active" : ""}`} onClick={() => setMode("private")}>
                    🔒 Private
                  </button>
                </div>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {[
                  { label: "Projects", value: portfolio.content?.projects?.length ?? 0 },
                  { label: "Skills", value: portfolio.content?.skills?.length ?? "—" },
                  {
                    label: "Languages", value: portfolio.content?.projects
                      ? [...new Set(portfolio.content.projects.flatMap(p => p.languages ?? []))].length
                      : "—"
                  },
                ].map(({ label, value }) => (
                  <div key={label} className="pf-card" style={{ padding: "14px 28px", textAlign: "center", minWidth: 140 }}>
                    <p style={{ fontSize: 26, fontWeight: 700, color: "#0d9488", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", margin: 0 }}>{value}</p>
                    <p style={{ fontSize: 11, color: "#94a3b8", margin: "4px 0 0", textTransform: "uppercase", letterSpacing: "0.06em" }}>{label}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 48, background: "linear-gradient(to bottom, transparent, #f8f7f4)" }} />
          </div>
