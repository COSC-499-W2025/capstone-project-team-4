...
def analyze_contributors(
    ...
    logger.info(
        f"analyze_contributors called with max_commits={max_commits}, use_all_branches={use_all_branches}"
    )
    ...
    try:
        ...
        for commit in commit_iter:
            ...
            try:
                raw_name = (
                    commit.author.name.strip() if commit.author.name else "Unknown"
                )
            ...
            canonical_name, canonical_email = _apply_mailmap(
                mailmap, raw_name, raw_email
            )

            # Aggregate stats per author
            author_key = (
                canonical_name or raw_name,
                _normalize_email(canonical_email or raw_email),
            )
            ...
            try:
                ...
                stats_output = repo.git.show(
                    commit.hexsha, numstat=True, no_renames=True, format=""
                )
                ...
                logger.debug(
                    f"Error collecting stats for commit {commit.hexsha[:8]}: {e}"
                )
    ...
    for contributor in contributors:
        if total_commits > 0:
            contributor["commit_percent"] = round(
                (contributor["commits"] / total_commits) * 100, 2
            )
        else:
    ...
    logger.info(
        f"Returning {len(result)} contributors, total_commits from sum: {total_commits}"
    )
    for c in result:
...
def _apply_mailmap(
    mailmap: Dict[Tuple[str, str], Tuple[str, str]], name: str, email: str
) -> Tuple[str, str]:
...
def _cluster_contributors(
    ...
    for cluster_identities in clusters.values():
        ...
        github_username = (
            sorted(all_github_usernames)[0] if all_github_usernames else None
        )
        ...
        for identity in cluster_identities:
            author_key = (
                ...
                _normalize_email(
                    identity.get("canonical_email") or identity.get("email")
                ),
            )
            ...
            for name1 in info1["names"]:
                for name2 in info2["names"]:
                    if _names_are_similar(name1, name2):
                        logger.debug(
                            f"Merging clusters due to similar names: '{name1}' ~ '{name2}'"
                        )
                        union(key1, key2)
                        break
                ...
                for user2 in info2["email_usernames"]:
                    ...
                    if user1 and user2 and len(user1) >= 4 and len(user2) >= 4:
                        if user1 in user2 or user2 in user1:
                            ...
                            if any(
                                _names_are_similar(n1, n2)
                                for n1 in info1["names"]
                                for n2 in info2["names"]
                            ):
                                logger.debug(
                                    f"Merging clusters due to username overlap: '{user1}' ~ '{user2}'"
                                )
                                union(key1, key2)
                                break
