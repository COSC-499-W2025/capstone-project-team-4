"""
Contribution ranking module.

Provides functionality to rank projects by contributor impact.

Migrated from src/core/contribution_ranking.py
"""
...
def compute_contribution_score(
    ...
    *,
    ...
    """
    Compute a numeric score from a contributor record.

    Weight is based on number of commits, lines changed, and files touched.

    Args:
        contributor: Contributor record from analyze_contributors()
        weight_commits: Weight for commit count
        weight_lines_changed: Weight for lines changed
        weight_files_touched: Weight for files touched
...
def _find_contributor(
    ...
    *,
    ...
    Args:
        ...
        match_by: Field to match ("name" or "email")
        value: Value to match

    Returns:
        Contributor record or None
    """
...
def rank_projects_for_contributor(
    ...
    *,
    ...
    Args:
        project_roots: Iterable of paths to project directories (each containing a .git folder)
        match_by: Either "name" or "email" - determines how the contributor is identified
        identifier: The exact name or email value to match (case-insensitive)
        score_kwargs: Optional scoring keyword arguments for compute_contribution_score()

    Returns:
        List of ProjectContributionSummary sorted by contribution_score descending
    ...
    The higher the score, the more significant the contributor's impact on that project.
    """
    ...
    for root in project_roots:
        ...
        contributors = analyze_contributors(str(root_path))
...
def summarize_top_projects(
    ...
    Args:
        ranked_projects: List of ProjectContributionSummary objects
        top_n: Number of top projects to summarize
