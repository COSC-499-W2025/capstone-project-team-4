...
class AnalysisService:
    ...
    def analyze_from_zip(
        ...
        # 1. Create temp dir manually (replaces Context Manager)
        ...
        try:
            ...
            if project_path != temp_path:
                logger.info(
                    f"Detected project root: {project_path.relative_to(temp_path)}"
                )
            else:
            ...
            # 2. Robust Cleanup in the 'finally' block
            self._force_cleanup(temp_dir)

    def _force_cleanup(self, path: str) -> None:
        """
        Force delete folder on Windows, retrying on locks.
        """
        max_retries = 5
        path_obj = Path(path)
        ...
        for i in range(max_retries):
            try:
                ...
                if getattr(e, "winerror", 0) == 32:
                    logger.warning(
                        f"File locked during cleanup, retrying ({i + 1}/{max_retries})..."
                    )
                    time.sleep(1)  # Wait for the lock to release
                else:
                    # If it's not a lock error (e.g. permission), try to chmod and delete
                    try:
                        import stat
                        import os
        ...
        logger.warning(f"Could not fully clean up temp dir after retries: {path}")
    ...
    def _run_analysis_pipeline(
        ...
        try:
            ...
            logger.info(
                f"Step 0 complete: Collected info for {len(file_info_list)} files in {stage_timings['file_collection']:.2f}s"
            )
            ...
            logger.info(
                f"Step 1 complete: {len(file_list)} files in {stage_timings['metadata_conversion']:.2f}s"
            )
            ...
            with ThreadPoolExecutor(max_workers=6) as executor:
                futures = {
                    executor.submit(
                        git_analyze_contributors, project_root
                    ): "contributors",
                    executor.submit(
                    ...
                    executor.submit(
                        detect_libraries_recursive, project_path
                    ): "libraries",
                    executor.submit(detect_tools_recursive, project_path): "tools",
                    executor.submit(
                        lambda: ProjectAnalyzer().analyze_project_languages(
                            project_root
                        )
                    ): "languages",
                    executor.submit(
                        self._detect_frameworks_best, project_path
                    ): "frameworks",
                }
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if task_name == "contributors":
                            contributors = result
                            logger.info(
                                f"Git analysis complete: Found {len(contributors)} contributors"
                            )
                        elif task_name == "complexity":
                            complexity_dict = result
                            logger.info(
                                f"Complexity analysis complete: Found {len(complexity_dict.get('functions', []))} functions"
                            )
                        elif task_name == "libraries":
                            library_report = result
                            logger.info(
                                f"Library detection complete: Found {library_report.get('total_count', 0)} libraries"
                            )
                        elif task_name == "tools":
                            tool_report = result
                            logger.info(
                                f"Tool detection complete: Found {tool_report.get('total_count', 0)} tools"
                            )
            ...
            logger.info(
                f"Steps 3-6 complete: Parallel analysis finished in {stage_timings['parallel_analysis']:.2f}s"
            )
            ...
            logger.info(
                f"Step 7 complete: Database saves finished in {stage_timings['database_saves']:.2f}s"
            )
            ...
            logger.info(
                f"Step 8 complete: Resume generated in {stage_timings['resume_generation']:.2f}s"
            )
    ...
    def _detect_project_root(self, base_path: Path) -> Path:
        ...
        if git_dirs:
            ...
            logger.info(
                f"Found .git directory at {project_root.relative_to(base_path)}"
            )
            return project_root
    ...
    def _extract_commit_dates_from_git(self, project_path: Optional[str]) -> List:
        ...
        try:
            ...
            # CRITICAL FIX: Explicitly close the repo handle to prevent Windows file locks
            if repo:
                try:
                    repo.close()
                except Exception:
