...
class TestFileStats:
    ...
    def test_file_stats_operations(self):
        ...
        stats = FileStats(
            files=5, total_lines=100, code_lines=80, comment_lines=15, blank_lines=5
        )
        assert (stats.files, stats.total_lines, stats.code_lines) == (5, 100, 80)

        # Test addition
        stats1 = FileStats(
            files=2, total_lines=50, code_lines=40, comment_lines=5, blank_lines=5
        )
        stats2 = FileStats(
            files=3, total_lines=75, code_lines=60, comment_lines=10, blank_lines=5
        )
        stats1.add(stats2)
...
class TestFileWalker:
    ...
    def test_should_analyze_file(self):
        ...
        # Test valid file
        with (
            patch.object(FileUtils, "get_file_info", return_value=("test.py", ".py")),
        ...
        with (
            patch.object(FileUtils, "get_file_info", return_value=(".hidden", "")),
            patch.object(FileUtils, "get_file_size", return_value=1000),
        ):
        ...
        with (
            patch.object(FileUtils, "get_file_info", return_value=(".gitignore", "")),
    ...
    def test_should_analyze_file_skip_patterns(self):
        ...
        with (
            patch.object(FileUtils, "get_file_info", return_value=("app.exe", ".exe")),
            patch.object(FileUtils, "get_file_size", return_value=1000),
        ):
        ...
        with (
            patch.object(
                FileUtils, "get_file_info", return_value=("package-lock.json", ".json")
            ),
            patch.object(FileUtils, "get_file_size", return_value=1000),
        ):
...
class TestFileAnalyzer:
    ...
    def test_language_detection(self, file_analyzer):
        ...
        with patch.object(
            FileUtils, "get_file_info", return_value=("script.py", ".py")
        ):
            assert (
                file_analyzer.detect_language_by_extension("/path/script.py")
                == "Python"
            )
        ...
        with patch.object(FileUtils, "get_file_info", return_value=("dockerfile", "")):
            assert (
                file_analyzer.detect_language_by_extension("/path/Dockerfile")
                == "Dockerfile"
            )
    ...
    def test_analyze_single_file_integration(self, file_analyzer):
        ...
        with (
            patch.object(
                file_analyzer, "detect_language_by_extension", return_value="Python"
            ),
            patch.object(
                file_analyzer,
                "count_lines_of_code",
                return_value=FileStats(files=1, total_lines=10, code_lines=8),
            ),
        ):
...
class TestProjectAnalyzer:
    ...
    def test_analyze_project_languages(self, project_analyzer):
        ...
        with (
            patch.object(
                project_analyzer.file_walker,
                "walk_source_files",
                return_value=["/proj/file1.py", "/proj/file2.py", "/proj/file3.js"],
            ),
            patch.object(
                project_analyzer.file_walker, "should_analyze_file", return_value=True
            ),
            patch.object(
                project_analyzer.file_analyzer,
                "detect_language_by_extension",
                side_effect=["Python", "Python", "JavaScript"],
            ),
        ):
    ...
    def test_analyze_project_lines_of_code(self, project_analyzer):
        ...
        stats1 = FileStats(
            files=1, total_lines=50, code_lines=40, comment_lines=5, blank_lines=5
        )
        stats2 = FileStats(
            files=1, total_lines=30, code_lines=25, comment_lines=3, blank_lines=2
        )
        ...
        with (
            patch.object(
                project_analyzer.file_walker,
                "walk_source_files",
                return_value=["/proj/file1.py", "/proj/file2.py"],
            ),
            patch.object(
                project_analyzer.file_walker, "should_analyze_file", return_value=True
            ),
            patch.object(
                project_analyzer.file_analyzer,
                "analyze_single_file",
                side_effect=[("Python", stats1), ("Python", stats2)],
            ),
        ):
            ...
            assert (
                python_stats.files,
                python_stats.total_lines,
                python_stats.code_lines,
            ) == (2, 80, 65)
    ...
    def test_get_unknown_files(self, project_analyzer):
        ...
        with (
            patch.object(
                project_analyzer.file_walker,
                "walk_source_files",
                return_value=["/proj/file1.xyz", "/proj/file2.abc", "/proj/file3.py"],
            ),
            patch.object(
                project_analyzer.file_walker, "should_analyze_file", return_value=True
            ),
            patch.object(
                project_analyzer.file_analyzer,
                "detect_language_by_extension",
                side_effect=["Unknown", "Unknown", "Python"],
            ),
        ):
            unknown_files = project_analyzer.get_unknown_files("/project", limit=5)
            assert len(unknown_files) == 2
            assert all(
                f in unknown_files for f in ["/proj/file1.xyz", "/proj/file2.abc"]
            )
    ...
    def test_analyze_empty_project(self, project_analyzer):
        ...
        with (
            patch.object(
                project_analyzer.file_walker, "walk_source_files", return_value=[]
            ),
            patch.object(
                project_analyzer.file_walker, "should_analyze_file", return_value=False
            ),
        ):
            # Test empty project
    ...
    def test_analyze_filtered_files(self, project_analyzer):
        ...
        with (
            patch.object(
                project_analyzer.file_walker,
                "walk_source_files",
                return_value=["/proj/file1.py", "/proj/file2.exe"],
            ),
            patch.object(
                project_analyzer.file_walker,
                "should_analyze_file",
...
class TestPublicAPI:
    ...
    def test_public_functions(self):
        ...
        with patch(
            "src.core.analyzers.language.ProjectAnalyzer"
        ) as mock_analyzer_class:
            mock_analyzer = MagicMock()
            mock_analyzer_class.return_value = mock_analyzer
            mock_analyzer.analyze_project_languages.return_value = {"Python": 5}
            mock_analyzer.analyze_project_lines_of_code.return_value = {
                "Python": FileStats(files=5, code_lines=400)
            }
            ...
            mock_analyzer.analyze_project_lines_of_code.assert_called_with(
                "/project", False
            )
...
class TestIntegration:
    ...
    def test_end_to_end_analysis(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (Path(temp_dir) / "test.js").write_text(
